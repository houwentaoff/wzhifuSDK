...
class WxPayConf_pub(object):
    ...
    if APPID is not  None:
        APPID = APPID.text
    #APPID = "wxd1dc60a5653dc3b4" #"wx8888888888888888"
    #JSAPI接口中获取openid，审核后在公众平台开启开发模式后可查看
    APPSECRET = root.find('appsecret')
    if APPSECRET is not None:
        APPSECRET = APPSECRET.text
    #APPSECRET = "d730fad976f9f4609f78e8b5da8dcc8a"#"48888888888888888888888888888887"
    #受理商ID，身份标识
...
class Common_util_pub(object):
    ...
    def postXmlCurl(self, xml, url, second=30):
        ...
        print  "url:", url, "xml:", self.xmlToArray(xml)
...
def test_order_query(out_trade_no):
    q = OrderQuery_pub()
    q.setParameter("out_trade_no", WxPayConf_pub.MCHID + out_trade_no)
    m = q.getResult()
    print '[wx]:-->test_order_query:', m
    if m['return_code'] == "FAIL":
...
def test_pay():
    p = UnifiedOrder_pub()
    p.setParameter("out_trade_no", "1212321321312")
    p.setParameter("body", "贡献一分钱")
    p.setParameter("total_fee", "1")
    p.setParameter("notify_url", "1212321321312")
    p.setParameter("trade_type", "NATIVE")
#    p.createXml()
    p.getPrepayId()
    c = HttpClient()
    assert c.get("http://www.baidu.com")[:15] == "<!DOCTYPE html>"
